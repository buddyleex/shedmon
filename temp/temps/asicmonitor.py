...
from minerips import antd3ips, ants9ips, innod9ips

def check_antminer_s9i(ip, name, port):
	try:
		...
		chain1_out = res[chain1+14:chain1+84]
		chain2_out = res[chain2+14:chain2+84]
		chain3_out = res[chain3+14:chain3+84]
...
def check_antminer_d3(ip, name, port):
        try:
                ...
                hash5s = int(res.find('GHS 5s'))
                chain1_out = res[chain1+14:chain1+81]
                chain2_out = res[chain2+14:chain2+81]
                chain3_out = res[chain3+14:chain3+81]
                chain1_f = int(chain1_out.find('x'))
                chain2_f = int(chain2_out.find('x'))
                chain3_f = int(chain3_out.find('x'))
